<template>
	<view class="image-cropper" :style="{ zIndex }" @wheel="cropper.mousewheel">
		...
		<view class="fixed-top" :style="{ zIndex: initData.area.zIndex + 99, paddingTop: `calc(${navbarHeight} + 20rpx)` }">
		</view>
		<view class="tip family-albb-bold u-font-28"
		:style="[{
			top: `calc(${navbarHeight} + 120rpx)`
		}]">
			{{tip}}
		</view>
		<view id="pic-preview" class="pic-preview" :change:init="cropper.initObserver" :init="initData" @touchstart="cropper.touchstart" @touchmove="cropper.touchmove" @touchend="cropper.touchend">
			<img v-if="imgSrc" id="crop-image" class="crop-image" :style="cropper.imageStyles" :src="imgSrc" webp></img>
		...
		<view class="fixed-bottom safe-area-inset-bottom" :style="{ zIndex: initData.area.zIndex + 99 }">
			<view class="action-box " v-if="(rotatable || reverseRotatable) && !!imgSrc">
				<view class="action-bar"
				@click="cropper.rotateImage90">
					<view class="rotate-icon" ></view>
					<!-- <view v-if="rotatable" class="rotate-icon is-reverse" ></view> -->
					<text>旋转90°</text>
				</view>
				<!-- <view class="ratio-box " >
					<view class="ratio-item r9-16"
					:class="{active: ratio === '9:16'}"
					@click="handleRatio('9:16')">

					</view>
					<view class="ratio-item r3-4"
					:class="{active: ratio === '3:4'}"
					@click="handleRatio('3:4')">
			...
			class="menu-bar">
				...
				<block v-else-if="!!imgSrc">
					...
					<button class="button" size="mini" @click="cropClick">完成</button>
				</block>
...
<script>
	...
	export default {
		...
		methods: {
			/** 提供给wxs调用，用来接收图片变更数据 */
			dataChange(e) {
				// console.log('dataChange', e, this.oldHeight, this.oldWidth)
				this.scaleWidth = e.width;
				this.scaleHeight = e.height;
				this.rotate = e.rotate;
				this.offsetX = e.x;
				this.offsetY = e.y;
				if(e.height >= this.oldHeight * 1.5){
					this.tip = '请避免人物在画面中高度不足一半'
				}else{
					this.tip = '请将身体置于人像框内'
				}
			},
			...
			initImage(url, isFirst) {
				uni.getImageInfo({
					src: url,
					success: async (res) => {
						if (isFirst && this.src === url) await (new Promise((resolve) => setTimeout(resolve, 100)));
			...
			handleRatio(ratio){
				...
				if(ratio === '3:4'){
					...
					const targetWidth = 3 * (this.height / 4);
					this.$emit('changeCropProperty', {
						width: targetWidth,
						height: targetHeight
					})
				}else{
					// 保持高度不变，裁剪宽度
					const targetHeight = this.height;
					const targetWidth = 9 * (this.height / 16);
					this.$emit('changeCropProperty', {
						width: targetWidth,
						height: targetHeight
					})

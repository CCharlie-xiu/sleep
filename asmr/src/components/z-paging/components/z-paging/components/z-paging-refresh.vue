    ...
    >
      ...
      <view class="zp-r-right">
        ...
        <text
          ...
          v-if="status !== R.Loading"
          :style="[rightTextStyle, titleStyle]"
          >{{ currentTitle }}</text
        >
        <view class="loading" v-else>
          <tn-icon
            :name="`${$staticPath.commonLocal}/lazy-load.png`"
            img-mode="widthFix"
            :customStyle="{
              width: '90rpx',
              height: '42rpx',
...
export default {
  ...
  computed: {
    ...
    leftImageClass() {
      ...
      return `zp-r-left-image ${preSizeClass} ${
...
.loading {
  animation: loading-pulse 1.5s cubic-bezier(0.4, 0, 0.6, 1) infinite;
}
// 加载图标脉冲动画

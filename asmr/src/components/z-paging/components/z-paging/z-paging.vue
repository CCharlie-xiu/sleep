		...
		<view :class="{'zp-view-super':true,'zp-scroll-view-super':!usePageScroll}" :style="[finalScrollViewStyle]">
			...
			<view :class="{'zp-scroll-view-container':true,'zp-absoulte':finalIsOldWebView}" :style="[scrollViewContainerStyle]">
				<scroll-view
					...
					:scroll-into-view="scrollIntoView" :lower-threshold="finalLowerThreshold" :upper-threshold="finalUpperThreshold"
					...
					>	
						...
						<view class="zp-paging-main" :style="[finalPagingStyle, scrollViewInStyle,{'transform': finalRefresherTransform,'transition': refresherTransition}]"
						<!-- #ifdef APP-VUE || MP-WEIXIN || MP-QQ || H5 -->
						...
						>	
							...
							<view class="zp-paging-container" :style="[{justifyContent:useChatRecordMode?'flex-end':'flex-start'}]">
								...
								<!-- <slot v-if="showLoading" name="loading">
									<view class="zp-loading">
										<view class="tn-flex tn-flex-column items-center justify-center">
											<image
												:src="`${$staticPath.commonLocal}/loading.png`"
												:style="{
													width: '204rpx',
													height: '151rpx',
												}"
...
<script module="pagingRenderjs" lang="renderjs">
	 ...
	 * 以下为自定义新增
	 * @property {Number|String} upperThreshold 距顶部/左边多远时，触发scrolltoupper事件，默认单位为px，默认为5px
	 */
	export default {
		name:"z-paging",
		// #ifdef APP-VUE || H5

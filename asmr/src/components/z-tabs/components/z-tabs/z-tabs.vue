		...
		<view ref="z-tabs-scroll-view-conatiner" class="z-tabs-scroll-view-conatiner">
			<scroll-view ref="z-tabs-scroll-view" class="z-tabs-scroll-view" :scroll-x="true" :scroll-left="scrollLeft" :show-scrollbar="false" :scroll-with-animation="isFirstLoaded" @scroll="scroll">
				<view class="z-tabs-list-container" :style="[tabsListStyle]">
					<view class="z-tabs-list w-100" :style="[tabsListStyle, 
          {marginTop: -finalBottomSpace+'px'}, 
          {'justify-content': tabJustify},
          <!-- #ifdef MP-KUAISHOU -->
          {height: unit==='rpx' ? '80rpx' : '40px'},
          <!-- #endif -->
          ]">
						<view :ref="`z-tabs-item-${index}`" :id="`z-tabs-item-${index}`" class="z-tabs-item"  :style="{...(currentIndex === index ? activeBoxColor : inactiveBoxColor),...tabStyle }" v-for="(item,index) in list" :key="index" @click="tabsClick(index,item)">
					...
					v-if="bar"
					:style="[{width: tabsContainerWidth+'px', bottom: finalBottomSpace+'px'}]">
 ...
 * @property {Boolean} bar 是否显示底部bar
...
export default {
  ...
  props: {
    ...
    bar: {
      type: Boolean,
      default: _gc("bar", true),
    },
    //自定义每个tab的内边距，默认为0 20rpx
    tabGap: {
      type: String,
      default: "0 20rpx",
    },
  ...
  mounted() {
    ...
    // 兼容vue3 通过ref获取不到组件实例的写法
    this.$emit("handleMounted", {
      setDx: this.setDx,
      unlockDx: this.unlockDx,
    });
  },
  watch: {
    current: {
  ...
  methods: {
    ...
    setDx(dx) {
      ...
      if (isLineMode) {
        this.bottomDotX =
          ...
          (nextBottomX - this.bottomDotXForIndex) * Math.abs(dxRate);
      } else if (isWormMode) {
        if (
          (isRight && currentIndex >= this.itemNodeInfos.length - 1) ||
          (!isRight && currentIndex <= 0)
        )
          return;
        const spaceOffset = isRight
          ? nextNodeInfo.right - currentNodeInfo.left
          : currentNodeInfo.right - nextNodeInfo.left;
        ...
        if (isRight) {
          if (barCalcedWidth > nextBottomX - this.bottomDotX + barWidth) {
            ...
            this.bottomDotX =
              this.bottomDotXForIndex + (barCalcedWidth - barMinusWidth) / 2;
            barCalcedWidth = barMinusWidth;
          }
        } else if (!isRight) {
          if (
            barCalcedWidth >
            this.bottomDotXForIndex + barWidth - nextBottomX
          ) {
            const barMinusWidth = barWidth + spaceOffset * (1 + dxRate);
            barCalcedWidth = barMinusWidth;
            ...
            this.bottomDotX =
              this.bottomDotXForIndex - (barCalcedWidth - barWidth);
          }
        }
    ...
    updateSubviewLayout(tryCount = 0) {
      this.$nextTick(() => {
        ...
        // #endif
        setTimeout(() => {
          this._getNodeClientRect(".z-tabs-scroll-view-conatiner").then(
            (res) => {
              if (res) {
    ...
    _updateDotPosition(index) {
      ...
      setTimeout(async() => { 
        ...
        if (JSON.stringify(this.activeStyle) !== "{}") {
          const nodeRes = await this._getNodeClientRect(
            `#z-tabs-item-${index}`,
            true
          );
          if (nodeRes) {
            ...
            this.tabsHeight = Math.max(
              node.height + rpx2px(28),
              this.tabsHeight
            );
            tabsContainerWidth = 0;
        ...
        if (node) {
          this.bottomDotX = this._getBottomDotX(
            node,
            this.finalBarWidth,
            offset
          );
        ...
        if (this.tabsWidth) {
          // setTimeout(()=>{
          let scrollLeft =
            this.bottomDotX - this.tabsWidth / 2 + this.finalBarWidth / 2;
          scrollLeft = Math.max(0, scrollLeft);
          if (tabsContainerWidth) {
            scrollLeft = Math.min(
              scrollLeft,
              tabsContainerWidth - this.tabsWidth + 10
            );
          }
      ...
      if (this.initTriggerChange && this.changeTriggerFailed && newVal.length) {
        if (this.current < newVal.length) {
          this.$emit(
            "change",
            this.current,
            newVal[this.current][this.valueKey]
          );
        }
      }
    },

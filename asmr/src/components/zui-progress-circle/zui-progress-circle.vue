<template>
  <view ref="eleMeter" :class="['zui-progress-circle', debug ? 'debug' : '']" :style="style">
    <view class="zui-progress-circle-wrapper">
      ...
      <image class="zui-progress-circle-ring zui-progress-circle-ring-active" mode="aspectFit" :src="svgDataUrl" />
      <image class="zui-progress-circle-ring" mode="aspectFit" :src="svgBgDataUrl" />
    </view>
...
export default {
  ...
  computed: {
    ...
    bgTextureFG() {
      ...
      if (typeof this.bgTexture === "string") {
        return this.parseTexture(this.bgTexture, textureSize);
      } else if (
        Object.prototype.toString.call(this.bgTexture) === "[object Array]"
      ) {
        if (typeof this.bgTexture[0] === "number") {
          return this.parseTexture(this.bgTexture, textureSize);
        } else {
          return this.parseTexture(this.bgTexture[0], textureSize);
        }
      } else {
        // use default texture
        return this.parseTexture("#1BB507", textureSize);
    ...
    bgTextureBG() {
      ...
      if (typeof this.bgTexture === "string") {
        ...
        if (typeof this.bgTexture[0] === "number") {
          ...
          return this.parseTexture(this.bgTexture[1], textureSize);
        }
      } else {
        // use default texture
        return this.parseTexture("#E2D8D8", textureSize);
  ...
  <g>
     ...
     */
    createBGSVG() {
      const cirleBG = this.hasBgBackground
        ? this.createCircle(this.bgTextureBG, this.generateDashArray(1))
        : "";
      const cirleFG = this.createCircle(
        this.bgTextureFG,
        this.generateDashArray(1),
        "fg"
      );
      const defs = [this.bgTextureFG.def || "", (this.bgTextureBG && this.bgTextureBG.def) || ""];
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${this.size}" height="${this.size}">
  <defs>
    ${defs.join("\n")}
  </defs>
...
.zui-progress-circle-ring-active {
  position: absolute;
  z-index: 1;
}

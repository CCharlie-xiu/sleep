      ...
      >
        <view
          ...
          :style="{
            ...
            marginRight:
              columnIndex < gridColumnsArr.length - 1
                ? skeletonConfigs.gridColumnsGap
                : 0,
          }"
        >
          <view
            ...
            :style="{
              ...
              marginBottom:
                ...
                skeletonConfigs.textShow
                  ? skeletonConfigs.itemGap
                  : 0,
              margin: skeletonConfigs.margin,
            }"
          ></view>
          <view
            v-if="skeletonConfigs.textShow"
            class="x-skeleton__wrapper__text"
          >
            <view
  ...
  computed: {
    ...
    textRowsArr() {
      ...
      for (let i = 0; i < this.skeletonConfigs.textRows; i++) {
        ...
        let item = {},
          // 需要预防超出数组边界的情况
          rowWidth = this.isArray(textWidth)
            ? textWidth[i] || (i === gridRows - 1 ? "70%" : "100%")
            : i === gridRows - 1
            ? "70%"
            : textWidth,
          rowHeight = this.isArray(textHeight)
            ? textHeight[i] || "30rpx"
            : textHeight;
        // 非百分比的宽度时，调整像素单位
        if (/%$/.test(rowWidth)) {
...
@mixin background {
  background: linear-gradient(
    90deg,
    var(--var_bg_v59_color) 25%,
    var(--var_page_bg_color) 37%,
    var(--var_bg_v59_color) 50%
  );
